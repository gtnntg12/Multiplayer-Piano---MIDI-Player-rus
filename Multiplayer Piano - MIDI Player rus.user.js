// ==UserScript==
// @name         Multiplayer Piano - MIDI Player rus// ==UserScript==
...
var createButtons = function() {
    ...
    var repeatTxt = document.createTextNode("Повторить");
...
var playerLimited = function(username) {
    ...
    mppChatSend(PRE_LIMITED + " Вы должны сделать что-то, чтобы заработать это " + quoteString(username) + " так как вам больше не разрешено использовать бота");
}
...
var play = function(url) {
    ...
    if (!exists(url) || url == "") {
        stopLoadingMusic();
        mppChatSend(error + " URL-адрес MIDI не введен... " + WHERE_TO_FIND_MIDIS);
    } else {
...
}, TENTH_OF_SECOND);// @namespace    https://thealiendrew.github.io/
// @version      2.5.8
// @description  Plays MIDI files!
// @author       AlienDrew
// @license      GPL-3.0-or-later
