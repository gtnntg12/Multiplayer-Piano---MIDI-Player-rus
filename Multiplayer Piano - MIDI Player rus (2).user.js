// ==UserScript==
// @name         Multiplayer Piano - MIDI Player rus
...
const BOT_NAME = "MIDI Player рус";
...
const PRE_PLAY = PRE_MSG + "[Воспроизвести]";
const PRE_STOP = PRE_MSG + "[Остановить]";
...
const PRE_SUSTAIN = PRE_MSG + "[Удержание]";
...
const WHERE_TO_FIND_MIDIS = "Вы можете найти хорошие файлы MIDI для загрузки на https://bitmidi.com/ и https://midiworld.com/, или вы можете использовать свои собственные файлы MIDI через Google Диск/Dropbox и т.д. с прямой ссылкой на загрузку";
const NOT_OWNER = "Бот не является владельцем комнаты";
const NO_SONG = "В данный момент ничего не играет";
const LIST_BULLET = "• ";
...
var urlToBlob = function(url, callback) {
    ...
    fetch(url, {
        ...
        console.error("Обычный fetch не смог получить файл:", error);
        ...
        if (corsUrl != null) {
            ...
            fetch(corsUrl, {
                ...
                clearInterval(downloading);
                if (!response.ok) {
                    throw new Error("Ответ сети был неправильным");
                }
                return response.blob();
            }).then(blob => {
                stopLoadingMusic();
                ...
                console.error("Не удалось получить файл через CORS Anywhere API:", error);
                stopLoadingMusic();
                clearInterval(downloading);
                callback(null);
            });
...
var playFile = function(songFile) {
    ...
    if (exists(songFile)) {
        ...
        if (songFile.size <= MIDI_FILE_SIZE_LIMIT_BYTES) {
            ...
            } else mppChatSend(error + " Файл \"" + songFileName + "\" либо повреждён, либо это не файл MIDI");
        } else mppChatSend(error + " Файл \"" + songFileName + "\" слишком большой (больше " + MIDI_FILE_SIZE_LIMIT_BYTES + " байт), пожалуйста, выберите файл меньшего размера");
    } else mppChatSend(error + " Файл MIDI не найден");
}

// Создает кнопки воспроизведения, паузы, продолжения и остановки для бота.
...
// Показывает ограниченное сообщение для пользователя
var playerLimited = function(username) {
// отображает сообщение с их именем о том, что им запрещено использовать бота.
    mppChatSend(PRE_LIMITED + " Вы, должно быть, что-то сделали, чтобы получить это " + quoteString(username) + ", так как вам больше не разрешено использовать бота.");
}

// Если введена неверная команда, показывает это сообщение об ошибке
var cmdNotFound = function(cmd) {
var error = PRE_ERROR + " Неверная команда, " + quoteString(cmd) + " не существует";
